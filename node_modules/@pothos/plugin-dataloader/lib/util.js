...
_export(exports, {
    ...
    clearAllDataLoaders: function() {
        return clearAllDataLoaders;
    },
...
const getAllDataLoaders = (0, _core.createContextCache)(()=>new Set());
function addDataloader(context, loader) {
    const loaderSet = getAllDataLoaders(context);
    loaderSet.add(loader);
    return loader;
}
function clearAllDataLoaders(context) {
    const loaderSet = getAllDataLoaders(context);
...
function dataloaderGetter(loaderOptions, load, toKey, sort) {
    ...
    return (0, _core.createContextCache)((context)=>addDataloader(context, new _dataloader.default((keys)=>loader(keys, context), loaderOptions)));
}
function pathDataloaderGetter(loaderOptions, load, toKey, sort, byPath) {
    ...
    return (args, ctx, info)=>{
        ...
        const map = cache(ctx);
        if (!map.has(key)) {
            map.set(key, addDataloader(ctx, new _dataloader.default((keys)=>loader(keys, ctx, args, info), loaderOptions)));
        }
        return map.get(key);
    };
}
